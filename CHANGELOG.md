...
- Added local OpenCV/Python setup docs, requirements tracking, and JSON-emitting helper scripts for future visual duplicate analysis preparation.
...
- Fixed the startup crash caused by unstable derived Zustand selector snapshots in the results toolbar and selection state.
...
- Added the project open dialog with restore and scan-again flows, restoring saved source/workspace state and running saved audit requests directly from project snapshots.

## [0.50.40] - 2026-05-20

### Added

- Added the saved-project sidebar with project list refresh/loading/error/empty states, saved project summary cards, and a prominent header Projects button.
...
- Added the named-project IPC, preload, and renderer-client boundary so the renderer can list, create, save, load, delete, and mark active projects through typed APIs.
...
- Moved global table selection to store-owned selected row IDs while continuing to derive selected row objects, paths, and counts for the existing table and workflow interfaces.
